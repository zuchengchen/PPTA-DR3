...
if model_comp:
    chain_label = "_" + "_".join(CRN_Name[0]) + "_v_" + "_".join(CRN_Name[1])
# set up output label for chain
else:
    chain_label = "_" + "_".join(CRN_Name[0])
...
if not dir == "ppta15":
    ...
    by_group_dict = {
        key: selections.Selection(sel_by_group_factory(item)._sel_by_group)
        for key, item in psr_groupnoiselist_dict.items()
    }
    ...
    by_group_ecorr_dict = {
        key: selections.Selection(sel_by_group_factory(item)._sel_by_group)
        for key, item in psr_groupecorrlist_dict.items()
    }
...
wn = white_signals.MeasurementNoise(efac=efac_prior, selection=by_backend)

# EQUAD - TempoNest definition: sigma = sqrt((efac*sigma_0)**2 + (tnequad)**2)
log10_equad_prior = parameter.Constant()
wn += white_signals.TNEquadNoise(log10_tnequad=log10_equad_prior, selection=by_backend)
...
if not dir == "ppta15":
    ...
    wn += gp_signals.EcorrBasisModel(
        log10_ecorr=log10_ecorr_prior, selection=ecorr_selection
    )
    if dir == "uwl" or dir == "dr2":
        wn += gp_signals.EcorrBasisModel(
            log10_ecorr=log10_ecorr_prior,
            selection=no_selection,
            name="basis_ecorr_all",
        )
    if dir == "all":
        wn += gp_signals.EcorrBasisModel(
            log10_ecorr=log10_ecorr_prior,
            selection=global_ecorr_selection,
            name="basis_ecorr_all",
        )
...
if model_comp:
    ...
    crn = [crns_1, crns_2]
    crn_names = [
        "-".join([i_ for _, i_ in CRN_Name[0].items()]),
        "-".join([i_ for _, i_ in CRN_Name[1].items()]),
    ]
...
if bayesephem_1 or bayesephem_2:
    beph = deterministic_signals.PhysicalEphemerisSignal(
        use_epoch_toas=True, model="setIII_1980", sat_orb_elements=True
    )
    if bayesephem_1:
...
for psr in psrs:
    ...
    """
    rn_model, rn_lgA_prior, rn_gam_prior, priors = get_informed_rednoise_priors(
        psr, "red_noise", noisedict_p0015, noisedict_p9985, priors
    )
    ...
    rn = gp_signals.FourierBasisGP(
        rn_model, components=red_components, selection=no_selection, name="red_noise"
    )

    hf_model, hf_lgA_prior, hf_gam_prior, priors = get_informed_rednoise_priors(
        psr, "hf_noise", noisedict_p0015, noisedict_p9985, priors
    )
    ...
    hf = gp_signals.FourierBasisGP(
        hf_model, components=hf_components, selection=no_selection, name="hf_noise"
    )

    band_model, band_lgA_prior, band_gam_prior, priors = get_informed_rednoise_priors(
        psr, "band_noise_low", noisedict_p0015, noisedict_p9985, priors
    )

    bandmid_model, bandmid_lgA_prior, bandmid_gam_prior, priors = (
        get_informed_rednoise_priors(
            psr, "band_noise_mid", noisedict_p0015, noisedict_p9985, priors
        )
    )
    bandhigh_model, bandhigh_lgA_prior, bandhigh_gam_prior, priors = (
        get_informed_rednoise_priors(
            psr, "band_noise_high", noisedict_p0015, noisedict_p9985, priors
        )
    )
    ...
    bn_mid = gp_signals.FourierBasisGP(
        bandmid_model,
        components=band_components,
        selection=mid_freq,
        name="band_noise_mid",
    ...
    bn_high = gp_signals.FourierBasisGP(
        bandhigh_model,
        components=band_components,
        selection=high_freq,
        name="band_noise_high",
    )
    ...
    if not dir == "ppta15":
        ...
        if psr_groupnoiselist_dict[psr.name] is None:
            ...
            gn_lgA_prior = parameter.Uniform(
                np.max(gn_lgA_prior_mins), np.min(gn_lgA_prior_maxs)
            )
            gn_gamma_prior = parameter.Uniform(
                np.max(gn_gamma_prior_mins), np.min(gn_gamma_prior_maxs)
            )
            gn_model = gp_priors.powerlaw(log10_A=gn_lgA_prior, gamma=gn_gamma_prior)

            gn = FourierBasisGP_ppta(
                gn_model,
                fmax=1 / (30 * 86400),
    ...
    dm_model, dm_lgA_prior, dm_gam_prior, priors = get_informed_rednoise_priors(
        psr, "dm_gp", noisedict_p0015, noisedict_p9985, priors
    )
    ...
    chrom_model, chrom_lgA_prior, chrom_gam_prior, priors = (
        get_informed_rednoise_priors(
            psr, "chrom_gp", noisedict_p0015, noisedict_p9985, priors
        )
    )
    ...
    chrom_basis = gp_bases.createfourierdesignmatrix_chromatic(
        nmodes=chrom_components, idx=idx
    )
    ...
    wf = chrom_yearly_sinusoid(log10_Amp=log10_Amp_dm1yr, phase=phase_dm1yr, idx=2)
    ...
    sw, priors = get_informed_nearth_priors(
        psr, noisedict_p0015, noisedict_p9985, priors
    )
    ...
    wf = dm_gaussian(
        log10_Amp=log10_Amp, epoch=epoch_gauss, log10_sigma=log10_sigma_gauss, idx=2
    )
    ...
    wf = gaussian_20cm(
        log10_Amp=log10_Amp,
        epoch=epoch_gauss_20cm,
        log10_sigma=log10_sigma_gauss,
        nu1=1000,
        nu2=2000,
    )
    ...
    wf = gaussian_chrom_gaussian(
        log10_Amp=log10_Amp,
        epoch=epoch_gauss_20cm,
        log10_sigma=log10_sigma_gauss,
        nu_0=nu0_chrom_gaussian,
        log10_sigma_nu=log10_sigma_nu_gauss,
    )
    gauss_chrom_gauss = deterministic_signals.Deterministic(
        wf, name="gauss_chrom_gauss"
    )
    ...
    if psr.name == "J1713+0747" and psr.toas.min() < 57500 * 86400:
        ...
        psr.name == "J2145-0750"
        and psr.toas.min() < 56450 * 86400
        and psr.toas.max() > 56300 * 86400
    ):
        expdip = True
        num_dips = 1
    ...
    if expdip:
        ...
        for idip in range(num_dips):
            ...
            wf = chrom_exp_decay(
                log10_Amp=log10_Amp_exp,
                t0=t0_exp,
                log10_tau=log10_tau_exp,
                sign_param=-1.0,
                idx=idx[idip],
            )
    ...
    if psr_groupecorrlist_dict[psr.name] is not None:
        ecg = gp_signals.EcorrBasisModel(
            log10_ecorr=log10_ecorr_prior,
...
chainfiles = sorted(
    [sorted(glob.glob(chaindir + "{}*chain.npy".format(psr_.name)))[0] for psr_ in psrs]
)
