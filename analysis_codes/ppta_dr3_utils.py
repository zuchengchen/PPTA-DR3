...
psr_groupecorrlist_dict_dr2 = {
    "J0437-4715": ["CASPSR_40CM", "PDFB_20CM"],
...
psr_groupecorrlist_dict_uwl = {
    "J0437-4715": ["UWL_PDFB4_20CM", "UWL_sbA", "UWL_sbG"],
    #'J0613-0200': ['UWL_sbG'],
...
psr_groupecorr_dict_dict = {
    "dr2": psr_groupecorrlist_dict_dr2,
    "uwl": psr_groupecorrlist_dict_uwl,
    "all": psr_groupecorrlist_dict_all,
}


psr_groupnoiselist_dict_dr2 = {
    "J0437-4715": [
        "CASPSR_40CM"
    ],  # ,-->taken out after fixing number of harmonics for group noise 'PDFB_20CM'],
    #'J0613-0200': ['PDFB_40CM'],
    #'J1017-7156': ['PDFB_20CM', 'PDFB_40CM']
    #'J1600-3053': ['CASPSR_40CM', 'PDFB_20CM', 'PDFB_10CM'],
    #'J1643-1224': ['PDFB_10CM', 'PDFB_40CM']#,
    "J1713+0747": [
        "WBCORR_10CM"
    ],  # PDFB1_early_20CM, -->taken out after fixing number of harmonics for group noise'CPSR2_20CM'
    "J1909-3744": [
        "CPSR2_50CM"
    ],  # -->taken out after fixing number of harmonics for group noise#'CASPSR_40CM', 'PDFB1_1433', 'PDFB1_early_20CM'
}

psr_groupnoiselist_dict_uwl = {
    "J0437-4715": ["UWL_PDFB4_20CM", "UWL_sbA", "UWL_sbG"],
    #'J0613-0200': ['UWL_sbG'],
    "J1017-7156": ["UWL_sbA", "UWL_sbD"],  # UNCOMMENT ME
    "J1022+1001": ["UWL_sbE", "UWL_sbH"],
...
psr_groupnoise_dict_dict = {
    "dr2": psr_groupnoiselist_dict_dr2,
    "uwl": psr_groupnoiselist_dict_uwl,
    "all": psr_groupnoiselist_dict_all,
}
...
by_group_uwl = selections.Selection(
    sel_by_group_factory(
        [
            "UWL_sbA",
            "UWL_sbB",
            "UWL_sbC",
...
def update_tidy_noisedict(noisefiles, dir, dirnoise):
    ...
    for nf in noisefiles:
        ...
        if "n_earth" in noisedict.keys():
            noisedict["{}_n_earth".format(os.path.basename(nf).split("_")[0])] = (
                noisedict["n_earth"]
            )
        ...
        for grouppsr, grouplist_ in psr_groupecorr_dict_dict[dir].items():
            ...
            for group_ in grouplist_:
                ...
                if f"{grouppsr}_basis_ecorr_{group_}" in key:
                    new_key = key.replace(
                        f"basis_ecorr_{group_}", f"basis_ecorr_group_{group_}"
                    )
                    noisedict[new_key] = noisedict[key]
                ...
                if not group_done:
                    print(
                        "COULD NOT FIND {} TERM FOR {} (KEY IS {})".format(
                            group_, grouppsr, key
                        )
                    )
                    raise ValueError
                    ...
                    uwl_gnf = sorted(
                        glob.glob(
                            dirnoise + f"/uwl/noiseFiles_maxlike/{grouppsr}*.json"
                        )
                    )[0]
                    with open(uwl_gnf, "r") as uwl_nf:
                        ...
                        for k in keys__:
                            if (
                                f"{grouppsr}_basis_ecorr_{group_}" in k
                                or f"{grouppsr}_basis_ecorr_group_{group_}" in k
                            ):
                                noisedict[
                                    k.replace(
                                        f"basis_ecorr_{group_}",
                                        f"basis_ecorr_group_{group_}",
                                    )
                                ] = uwl_nd[k]
...
def get_3sig_noisedict(dir, dirnoise):
    ...
    if dir == "all":
        if len(glob.glob(dirnoise + "/all/noiseFiles/3sig/*p0015s.json")) > 0:
            noisedict_p0015 = {}
            noisefiles_p0015 = sorted(
                glob.glob(dirnoise + "/all/noiseFiles/3sig/*p0015s.json")
            )
            for nf in noisefiles_p0015:
                with open(nf, "r") as f:
                    ...
                    if "n_earth" in noisedict_p0015.keys():
                        noisedict_p0015[psrname + "_n_earth"] = noisedict_p0015[
                            "n_earth"
                        ]
                    del noisedict_p0015["n_earth"]
            noisedict_p9985 = {}
            noisefiles_p9985 = sorted(
                glob.glob(dirnoise + "/all/noiseFiles/3sig/*p9985s.json")
            )
            for nf in noisefiles_p9985:
                with open(nf, "r") as f:
                    ...
                    if "n_earth" in noisedict_p9985.keys():
                        noisedict_p9985[psrname + "_n_earth"] = noisedict_p9985[
                            "n_earth"
                        ]
            ...
            noisefiles_p9985_dr2 = sorted(
                glob.glob(dir_3sig + "/dr2/noiseFiles/3sig/*p9985s.json")
            )
            noisefiles_p0015_dr2 = sorted(
                glob.glob(dir_3sig + "/dr2/noiseFiles/3sig/*p0015s.json")
            )
            noisefiles_p9985_uwl = sorted(
                glob.glob(dir_3sig + "/uwl/noiseFiles/3sig/*p9985s.json")
            )
            noisefiles_p0015_uwl = sorted(
                glob.glob(dir_3sig + "/uwl/noiseFiles/3sig/*p0015s.json")
            )
            # print(noisefiles)
        ...
        dir_3sig = os.path.abspath("./data/")
        noisefiles_p9985 = sorted(
            glob.glob(dir_3sig + "/" + str(dir) + "/noiseFiles/3sig/*p9985s.json")
        )
        noisefiles_p0015 = sorted(
            glob.glob(dir_3sig + "/" + str(dir) + "/noiseFiles/3sig/*p0015s.json")
        )
        noisedict_p0015 = {}
        for nf in noisefiles_p0015:
            with open(nf, "r") as f:
            ...
            if "n_earth" in noisedict_p0015.keys():
                noisedict_p0015[
                    "{}_n_earth".format(os.path.basename(nf).split("_")[0])
                ] = noisedict_p0015["n_earth"]
                del noisedict_p0015["n_earth"]
        noisedict_p9985 = {}
        for nf in noisefiles_p9985:
            with open(nf, "r") as f:
...
def construct_psrs(
    ...
    for p, t in zip(parfiles, timfiles):
        ...
        if islice is not None:
            ...
            tnew = t.replace(
                ".tim", "_cut_{}_{}_{}.tim".format(islice, yearslice, chainnum)
            )
            command = "awk '{{if (($3 < {0}) || ($3 > {1})) print \"# \", $0; else print $0}}' {2}  > {3}".format(
                min_mjd, max_mjd, t, tnew
            )
            os.system(command)
            nlines = int(
                subprocess.check_output(
                    f'grep -v "#" {tnew} | wc -l', shell=True, text=True
                )
            )
            if nlines <= 1:
                print(
                    "{} Has <= 1 ToA after cutting. Continuing.".format(
                        p.split("/")[-1].split(".")[0]
                    )
                )
            ...
            try:
                ...
                print(e)
                print(
                    "{} Has no data after cutting. Continuing.".format(
                        p.split("/")[-1].split(".")[0]
                    )
                )
                os.system("rm {}".format(tnew))
...
def get_crn_model_dict(tspan):
    ...
    crn_model_dict = {
        ...
        "pl_nocorr_freegam": crn_block(
            psd="powerlaw",
            prior="log-uniform",
            gamma_val=None,
            components=components,
            orf=None,
            name="gw_pl_nocorr_freegam",
        ),
        # powerlaw, no correlation, fixed 13/3 spectral index
        "pl_nocorr_fixgam": crn_block(
            psd="powerlaw",
            prior="log-uniform",
            gamma_val=4.333,
            components=components,
            orf=None,
            name="gw_pl_nocorr_fixgam",
        ),
        # broken power law, no correlation
        "bpl_nocorr_freegam": crn_block(
            psd="broken_powerlaw",
            prior="log-uniform",
            components=60,
            orf=None,
            name="gw_bpl_nocorr_freegam",
        ),
        # free-spectrum no correlation
        "freespec_nocorr": crn_block(
            psd="spectrum",
            prior="log-uniform",
            components=40,
            orf=None,
            name="gw_freespec_nocorr",
        ),
        # power law red noise, ORF sampled in angular bins
        "pl_orf_bins": crn_block(
            psd="powerlaw",
            prior="log-uniform",
            gamma_val=4.333,
            components=components,
            orf="bin_orf",
            name="gw_apl_orf_bins",
        ),
        # power law red noise, ORF sampled in angular bins
        "pl_orf_singlebin": common_red_noise_block(
            psd="powerlaw",
            prior="log-uniform",
            gamma_val=4.333,
            components=components,
        ...
        "pl_orf_singlebin_fixA": common_red_noise_block(
            psd="powerlaw",
            prior="log-uniform",
            gamma_val=4.333,
            log10_A_val=-14.69,
        ...
        "pl_dipole_freegam": crn_block(
            psd="powerlaw",
            prior="log-uniform",
            components=components,
            orf="dipole",
            name="gw_pl_dipole_freegam",
            gamma_val=None,
        ),
        # free spectrum red noise, Dipole correlations
        "freespec_dipole": crn_block(
            psd="spectrum",
            prior="log-uniform",
            components=40,
            orf="dipole",
            name="gw_freespec_dipole",
        ),
        # power law red noise, free spectral index, Monopole correlations,
        "pl_monopole_freegam": crn_block(
            psd="powerlaw",
            prior="log-uniform",
            components=components,
            orf="monopole",
            name="gw_pl_monopole_freegam",
            gamma_val=None,
        ),
        # free spectrum red noise, Monopole correlations
        "freespec_monopole": crn_block(
            psd="spectrum",
            prior="log-uniform",
            components=40,
            orf="monopole",
            name="gw_freespec_monopole",
        ),
    }
...
def get_informed_rednoise_priors(
    psr,
    noisename,
    noisedict_3sig_min,
    noisedict_3sig_max,
    priors,
    return_priorvals=False,
    use_basic_priors=False,
    log10_A_min_basic=-20,
    log10_A_min_informed=-18,
    log10_A_min_bound=-2,
):
    ...
    if not use_basic_priors:
        try:
            log10_A_min = np.max(
                [log10_A_min_informed, noisedict_3sig_min[key_] + log10_A_min_bound]
            )
    ...
    print(
        f"""{psr.name}_{noisename}_noise prior:
    log10_A in [{log10_A_min}, {log10_A_max}]
    gamma in [{gamma_min}, {gamma_max}]
    """
    )
    # # powerlaw
    rednoise_model = gp_priors.powerlaw(log10_A=log10_A_prior, gamma=gamma_prior)
    key_ = psr.name + "_" + noisename + "_log10_A"
    priors[key_ + "_min"] = log10_A_min
    priors[key_ + "_max"] = log10_A_max
    key_ = psr.name + "_" + noisename + "_gamma"
        ...
        for i in range(0, ndim):
            ...
            while no_sample:
                ...
                if "_mid_mid" in params[i] or "_high_high" in params[i]:
                    param_new = param_new.replace("_mid_mid", "_mid").replace(
                        "_high_high", "_high"
                    )
                if len(ind_par) == 0:
                ...
                if (
                    not "dmexp" in params[i]
                    and not "dm1yr" in params[i]
                    and not "dmgaus" in params[i]
                ):
                    try:
                        if (
                            sample < priors[param_new + "_min"]
                            or sample > priors[param_new + "_max"]
                        ):
                            print(
                                "################ SAMPLE {} OUTSIDE OF PRIOR RANGE FOR {} ###############".format(
                                    sample, params[i]
                                )
                            )
                            continue
...
def get_chain(
    ...
    for i in range(1, max_nchain + 1):
        ...
        if load_existing and os.path.exists(
            "{}/chains/{}.npy".format(datadir, noise_model)
        ):
        ...
        print(chainfile)
        nlines = int(
            subprocess.check_output(f"cat {chainfile} | wc -l", shell=True, text=True)
        )
        if nlines < nburnmin:
            print(nlines, "dead")
            continue
        ...
        if plot_trace:
            ...
            pp.plot_trace()
            plt.savefig(
                chainfile.replace(".txt", "_{}_{}_trace.png".format(psrname, dir))
            )
            plt.close()
            print(
                "Made {}".format(
        ...
        inds = np.argwhere(
            [("nmodel" not in p and "gw" not in p and "bin" not in p) for p in pars]
        )
        thin = 1
        good_pars = 0
        for indi in inds:
